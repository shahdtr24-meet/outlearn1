...
import { cvWritingLessons } from '../data/cv-writing';
import { getLevelProgress, isLevelCompleted, isLevelUnlocked, resetProgress } from '../services/cvWritingProgress';

const { width } = Dimensions.get('window');

export default function CVWritingLevels() {
  ...
  const levels = cvWritingLessons.levels?.map((level, index) => {
    ...
    return {
      ...
      title: level.title,
      status,
      type: index === 0 ? 'start' : index === (cvWritingLessons.levels?.length || 0) - 1 ? 'end' : 'middle',
    };
  }) || [];
  ...
  const navigateToLevel = (levelNumber, status) => {
    ...
    // Navigate to the CV writing level explanation page
    router.push(`/courses/cv-writing-level?id=${levelNumber}`);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{cvWritingLessons.skillName}</Text>
          <Text style={styles.subtitle}>
            {cvWritingLessons.skillDescription}
          </Text>
        </View>
