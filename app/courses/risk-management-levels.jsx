...
import { riskManagementLessons } from '../data/risk-management';
import { getLevelProgress, isLevelCompleted, isLevelUnlocked, resetProgress } from '../services/riskManagementProgress';

const { width } = Dimensions.get('window');

export default function RiskManagementLevels() {
  ...
  const levels = riskManagementLessons.levels?.map((level, index) => {
    ...
    return {
      ...
      title: level.title,
      status,
      type: index === 0 ? 'start' : index === (riskManagementLessons.levels?.length || 0) - 1 ? 'end' : 'middle',
    };
  }) || [];
  ...
  const navigateToLevel = (levelNumber, status) => {
    ...
    // Navigate to the risk management level explanation page
    router.push(`/courses/risk-management-level?id=${levelNumber}`);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{riskManagementLessons.skillName}</Text>
          <Text style={styles.subtitle}>
            {riskManagementLessons.skillDescription}
          </Text>
        </View>
