...
import { purchaseManagementLessons } from '../data/purchase-management';
import { getLevelProgress, isLevelCompleted, isLevelUnlocked, resetProgress } from '../services/purchaseManagementProgress';

const { width } = Dimensions.get('window');

export default function PurchaseManagementLevels() {
  ...
  const levels = purchaseManagementLessons.levels?.map((level, index) => {
    ...
    return {
      ...
      title: level.title,
      status,
      type: index === 0 ? 'start' : index === (purchaseManagementLessons.levels?.length || 0) - 1 ? 'end' : 'middle',
    };
  }) || [];
  ...
  const navigateToLevel = (levelNumber, status) => {
    ...
    // Navigate to the purchase management level explanation page
    router.push(`/courses/purchase-management-level?id=${levelNumber}`);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{purchaseManagementLessons.skillName}</Text>
          <Text style={styles.subtitle}>
            {purchaseManagementLessons.skillDescription}
          </Text>
        </View>
